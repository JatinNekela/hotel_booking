...
const AddRoom = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            //convert data into amenities to array & keeping only enabled amenities
    ...
    return (
        <form onSubmit={onSubmitHandler}>
            <Title align='left' font='outfit' title='Add Room' subTitle='Fill in the details as per availability' />
            ...
            <div className="w-full flex max-sm:flex-col sm:gap-4 mt-4">
                ...
                <div className="">
                    ...
                    <input type='number' placeholder="0" className="border border-gray-300 mt-1 rounded p-2 w-full" value={inputs.pricePerNight}
            ...
            <div className="flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm">
                {Object.keys(inputs.amenities).map((amenity, index) => (
                    <div key={index}>
                        <input type='checkbox' id={`ammenities${index + 1}`} checked={inputs.amenities[amenity]}
                            onChange={e => setInputs({ ...inputs, amenities: { ...inputs.amenities, [amenity]: !inputs.amenities[amenity] } })} />
                        <label htmlFor={`ammenities${index + 1}`}> {amenity}</label>
                    </div>
                ))}
            </div>
