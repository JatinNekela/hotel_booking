...
const AllRooms = () => {
    ...
    //function to sort rooms based on the selected sort function
    const sortRooms = (a,b) => {
    ...
    //filter and sort rooms based on the selected filters and option
    const filteredRooms = useMemo(() => {
        return rooms.filter(room => matchesRoomTypes(room) && matchesPriceRange(room) && filterDestination(room)).sort(sortRooms);
            ...
            <div>
                ...
                {filteredRooms.map((room) => (
                    <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last-border-0'>
                        <img onClick={() => { navigate(`/rooms/${room._id}`); scrollTo() }}
                            src={room.images[0]} alt="hotel-img" title='View Room details ' className='max-h-64 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer' />
                        <div>
                            <p className='text-gray-500'>{room.hotel.city}</p>
                            <p onClick={() => { navigate(`/rooms/${room._id}`); scrollTo() }} className='text-gray-800 text-3xl font-playfair cursor-pointer'>{room.hotel.name}</p>
                            ...
                            <div className='flex items-center gap-1 text-grey-500 mt-2 text-sm'>
                                ...
                                <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}> <img src={assets.locationIcon} alt="location-icon" />  {room.hotel.address}</span>
                            </div>
                            {/*Room Ammenities*/}
